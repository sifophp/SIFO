...
function smarty_function_fill($params, &$smarty)
{
	...
	foreach( $params as $_key => $_val )
	{
		...
		if ( true === Urls::$normalize_values && ( !isset($params['lower'] ) || $params['lower'] != 'no' ) )
		{
			$_html_result = str_replace( $_delimiter . $_key . $_delimiter, Urls::normalize( (string)$_val ), $_html_result);
		}
		else
		{
