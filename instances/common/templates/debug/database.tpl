...
{foreach name=queries from=$debug.queries item=query}
	<h3 class="queries {if false !== $query.error}query_error{else}query_{$query.type}{/if} {if $query.duplicated}query_duplicated{/if}{if $query.time >= 0.5 && $query.time < 1} query_slow{elseif $query.time >= 1} query_very_slow{/if}" id="queries_{$smarty.foreach.queries.index}">
