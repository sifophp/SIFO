...
{foreach name=controllers from=$debug.controllers item=controller key=controller_name}
	<h3 id="cont_{$smarty.foreach.controllers.index}"><a class="debug_toggle_view" rel="cont_content_{$smarty.foreach.controllers.index}{$execution_key|default:''}" href="#">{$smarty.foreach.controllers.index+1}. {$controller_name}</a></h3>
	<div id="cont_content_{$smarty.foreach.controllers.index}{$execution_key|default:''}" class="debug_contents">
{	foreach from=$controller item=content key=key name=controllerparams}
{		if $key == "CONTROLLER"}
	<h4><a class="debug_toggle_view" rel="params_cont_content_{$smarty.foreach.controllers.index}_{$smarty.foreach.controllerparams.index}{$execution_key|default:''}" href="#">Controller parameters</a></h4>
	<div id="params_cont_content_{$smarty.foreach.controllers.index}_{$smarty.foreach.controllerparams.index}{$execution_key|default:''}" class="debug_contents">
	<ul>
...
{*	TEMPLATES AND OTHER FUTURE ELEMENTS *}
	<h4><a class="debug_toggle_view" rel="assigns_cont_content_{$smarty.foreach.controllers.index}_{$smarty.foreach.controllerparams.index}{$execution_key|default:''}" href="#">
{			if $key == "assigns"}Template assigns{else}{$key}{/if}
		</a></h4>
	<div  id="assigns_cont_content_{$smarty.foreach.controllers.index}_{$smarty.foreach.controllerparams.index}{$execution_key|default:''}" class="debug_contents">
	<ul>
