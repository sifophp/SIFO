...
class Database
{
    ...
    // Input in lower case:
    private $methods_whitout_duplicated_validation = [
        'prepare',
        'affected_rows',
        'insert_id',
        'errorno',
    ...
    ) //call adodb methods
    {
        ...
        if (self::TYPE_SINGLE_SERVER != self::$destination_type) {
            ...
            if (in_array($method, ['Affected_Rows', 'Insert_ID'])) {
        ...
        try {
            $answer = call_user_func_array([self::$adodb[self::$destination_type], $method], $args);
        } catch (\ADODB_Exception $e) {
            $answer = false;
            $error = $e->getMessage();
    ...
    protected function queryDebug(
        ...
        $debug_query = [
            ...
            "resultset" => is_integer($resultset) ? [[$method => $resultset]] : $resultset,
            "time" => $query_time,
