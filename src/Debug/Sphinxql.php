...
use Sifo\Benchmark;
use Sifo\Database\Sphinx\Sphinxql as OriginalSphinxql;

/**
 * SphinxQL Debug class.
 */
class Sphinxql extends OriginalSphinxql
{
    /**
    ...
    public function getCallerClass()
    {
        ...
        foreach (array_reverse($array_debug) as $debug_step)
        {
            ...
            ++$step;
            $trace[] =
                "$step > " . $debug_step['class'] . '::' . $debug_step['function'] . ' - ' . basename($debug_step['file']) . ':' . $debug_step['line'] . " [" . dirname(
                    $debug_step['file']
                ) . "]";
