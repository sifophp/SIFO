...
class DependencyInjector implements ContainerInterface
{
        ...
        foreach ($arguments as $argument) {
            ...
            if ($this->isALiteralArgument($argument)) {
                ...
                foreach ($argument as $argument_key => $argument_value) {
                    $stringified_value = $this->stringifyArguments([$argument_value], $compiled_services,
                        $depth + 1)[0];
                    $compiled_argument .= str_repeat("\t",
                            $depth + 1) . '\'' . $argument_key . '\' => ' . ltrim($stringified_value) . ",\n";
                }
                ...
                $compiled_arguments[] = str_repeat("\t",
                        $depth) . "\$container->get('" . $dependant_service . "', true)";
            }
        }
    ...
    {
        $imports = is_array($parsed_yaml_content) ? array_key_exists('imports',
            $parsed_yaml_content) ? $parsed_yaml_content['imports'] : array() : array();

        $services = is_array($parsed_yaml_content) ? array_key_exists('services',
            $parsed_yaml_content) ? $parsed_yaml_content['services'] : array() : array();
