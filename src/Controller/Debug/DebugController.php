...
use Sifo\Debug\FilterCookieDebug;
...
class DebugController extends Controller
{
    ...
    {
        $total_script_execution = Benchmark::getInstance()->timingCurrent();
        ...
        Debug::setExecutionKey($this->execution_key);

        // Headers:
        $debug_data['headers'] = Headers::getDebugInfo();

        // Basic Debug data:
        $debug_data['controllers'] = Debug::get('controllers');
        $debug_data['benchmarks'] = Debug::get('benchmarks');
        $debug_data['elements'] = Debug::get('elements');

        // Execution times
        $debug_data['times'] = Debug::get('times');

        if (!isset($debug_data['times']['cache'])) {
            $debug_data['times']['cache'] = 0;
        }
        if (!isset($debug_data['times']['external'])) {
            $debug_data['times']['external'] = 0;
        }
        if (!isset($debug_data['times']['db_connections'])) {
            $debug_data['times']['db_connections'] = 0;
        }
        if (!isset($debug_data['times']['db_queries'])) {
            $debug_data['times']['db_queries'] = 0;
        }
        if (!isset($debug_data['times']['search'])) {
            $debug_data['times']['search'] = 0;
        }
        if (!isset($debug_data['times']['sphinxql'])) {
            $debug_data['times']['sphinxql'] = 0;
        }
        ...
        // Smarty Debug:
        $debug_data['smarty_errors'] = Debug::get('smarty_errors');

        // Database debug.
        $debug_data['queries'] = Debug::get('queries');
        $debug_data['queries_errors'] = Debug::get('queries_errors');
        $debug_data['queries_duplicated'] = Debug::get('duplicated_queries');

        // Search debug.
        $debug_data['search'] = Debug::get('searches');

        // SphinxQl debug.
        $debug_data['sphinxql'] = Debug::get('sphinxql');
        $debug_data['sphinxql_errors'] = Debug::get('sphinxql_errors');

        // Redis debug.
        $debug_data['redis'] = Debug::get('redis');
        ...
        // Debug messages:
        $debug_data['log_messages'] = Debug::get('log_messages');
    ...
    protected function finalRender($debug_data)
    {
        ...
        $database_debug_handler->saveExecutionDebug($this->execution_key, $this->getDebugUrl(), $debug_data,
            $this->getParam('executed_controller_is_json'));
    ...
    {
        return Urls::$base_url . FilterServer::getInstance()->getString('REQUEST_URI');
    }

    private function getPostData()
    {
        return FilterPost::getInstance()->getRawRequest();
    }
