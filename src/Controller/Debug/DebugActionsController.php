...
class DebugActionsController extends Controller
{
    public $is_json = true;
    ...
    public function __construct()
    {
        ...
        $this->filter_get = FilterGet::getInstance();
    }

    function build()
    {
        if (!Domains::getInstance()->getDevMode()) {
            throw new NotFound('Debug actions only available while in devel mode');
        }
    ...
    private function linkAction()
    {
        ...
        if (!$this->debug_persistence_handler->linkChildExecutionToParent($this->child_execution_key,
            $this->execution_key)
        ) {
            return array(
                'status' => 'KO',
