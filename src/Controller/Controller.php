    ...
    protected function parseParams()
    {
        ...
        if (count($expected_url_params)) {
            for ($i = 0; $i < $max; $i++) {
                ...
                if ((3 >= strlen($param))
                    &&  // Can be a expected key
                    (filter_var($param, FILTER_DEFAULT))
                    && // Is a valid string
                    (array_key_exists($param, $expected_url_keys))
                    && // Exists in getParamsDefinition params array.
                    (false !== filter_var($this->params['params'][$i + 1], FILTER_DEFAULT)
                        && '' != filter_var(
